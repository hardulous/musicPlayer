...
const Home = () => {
  ...
  const handleSubmit = async(type)=>{
    let response
    if(type=="signup"){
      response = await signUp(Cred.first_name,Cred.last_name,Cred.sig_email,Cred.sig_password);
    }
    else{
          ...
          <Box width="40%">
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <TextField
                ...
                onChange={handleChange}
              ></TextField>
              <Button variant="contained" color="success" startIcon={<Add />} onClick={()=>{handleSubmit("login")}}>
                Log in
              </Button>
            </Box>
          </Box>
